  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(collection(db, "busReports"), (snapshot) => {
      ...
      snapshot.docs.forEach((docSnap) => {
        ...
        if (!companyReportsMap[companyID]) companyReportsMap[companyID] = {};
        if (!companyReportsMap[companyID][reportId]) {
          companyReportsMap[companyID][reportId] = {
            data: null,
            latestMessage: null,
          };
        }

        // Listen to individual report changes
        const unsubReport = onSnapshot(doc(db, "busReports", reportId), (reportSnap) => {
          const reportData = reportSnap.data();
          companyReportsMap[companyID][reportId].data = reportData;
        ...
        // Listen to latest message per report
        const messagesRef = collection(db, "busReports", reportId, "messages");
        const unsubMsg = onSnapshot(
          query(messagesRef, orderBy("createdAt", "desc"), limit(1)),
          (msgSnap) => {
    ...
    // ✅ Fix applied here: only show unread if NOT the selectedReport
    function checkUnreadPerCompany(companyReportsMap, reportsByCompany) {
      const result = Object.keys(reportsByCompany).map((companyID) => {
        const reports = companyReportsMap[companyID];
        let hasUnread = false;
        ...
        for (const reportID in reports) {
          ...
          const unread =
            ...
            (msg?.senderRole === "operator" &&
              !msg?.seen &&
              !(selectedCompany === companyID && selectedReport?.id === reportID)); 
        ...
        return {
          ...
          hasUnreadMessages:
            ...
            (lastMessage?.senderRole === "operator" &&
             !lastMessage?.seen &&
             selectedReport?.id !== reportID), // ✅ Only show unread if not currently open
        };
      } catch (err) {
      ...
      try {
        if (file) {
          ...
          const storageRef = ref(
            storage,
            `busReportImages/${selectedReport.id}/${Date.now()}_${file.name}`
          );
