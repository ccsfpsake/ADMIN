...
const Sidebar = ({ isSidebarOpen, toggleSidebar }) => {
  ...
  useEffect(() => {
    ...
    // Real-time updates
    const unsub = onSnapshot(collection(db, "BusLocation"), (snapshot) => {
      checkIdleBuses(snapshot);
    });

    // Initial check (without waiting for snapshot to fire)
    getDocs(collection(db, "BusLocation")).then(checkIdleBuses);
